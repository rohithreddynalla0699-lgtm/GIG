...
import { getOrderListSupportHint, getOrderStatusLabel, getPartnerOrderQueueHint, getPartnerOrderRowClasses } from '../../lib/status';
    ...
    <div className="overflow-hidden rounded-[18px] border border-[rgba(32,38,28,0.08)] bg-white/78">
        ...
        <table className="w-full">
          ...
          <tbody>
            {orders.map((order) => {
              ...
              const queueHint = getPartnerOrderQueueHint(order.status);

              return (
                <tr key={order.id} className={`border-b border-[color:var(--gig-border)] last:border-b-0 ${getPartnerOrderRowClasses(order.status)}`}>
                  ...
                  <td className="px-4 py-3 align-top">
                    ...
                    {queueHint ? (
                      <div className="mt-1 text-[12px] font-semibold text-[color:var(--gig-text-soft)]">{queueHint}</div>
                    ) : null}
      ...
      <div className="space-y-3 p-3 xl:hidden">
        {orders.map((order) => {
          ...
          return (
            <div key={order.id} className={`rounded-[16px] border border-[rgba(32,38,28,0.08)] bg-white/74 px-4 py-3 ${getPartnerOrderRowClasses(order.status)}`}>
              ...
              {queueHint ? (
                <div className="mb-2 text-[12px] font-semibold text-[color:var(--gig-text-soft)]">{queueHint}</div>
              ) : null}
